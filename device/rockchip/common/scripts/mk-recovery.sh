...
build_hook()
{
	...
	if [ -e loong/devices/${BUILD_PROJECT}/buildroot ] ; then
		cp -rf -P loong/devices/${BUILD_PROJECT}/buildroot ./
		echo "\n ${BUILD_PROJECT} recovery overlay !\n"
	fi
	...
	if [ -e loong/devices/${BUILD_PROJECT}/buildroot ] ; then
		...
		git checkout .
		cd -
		echo "\n ${BUILD_PROJECT} recovery checkout !\n"
	fi
