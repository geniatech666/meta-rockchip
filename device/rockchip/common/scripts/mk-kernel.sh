...
do_build()
{
	...
	if [  -e loong/devices/${BUILD_PROJECT}/kernel ] ; then
		echo  "kernel overlay"
		echo "project name:$BUILD_PROJECT"
		cp -rf -P loong/devices/${BUILD_PROJECT}/kernel/ ./
	fi
	...
	if [[ "${BUILD_PROJECT}" == "rk3568j-xpi-yocto" ]]; then
		echo  "wifibt overlay"
                cp -rf -P kernel/drivers/net/wireless/rockchip_wlan/8897U/mbtc_src/*.ko loong/devices/${BUILD_PROJECT}/yocto-overlay/lib/modules/
		cp -rf -P kernel/drivers/net/wireless/rockchip_wlan/8897U/wlan_src/*.ko loong/devices/${BUILD_PROJECT}/yocto-overlay/lib/modules/
	fi
	...
	if [  -e loong/devices/${BUILD_PROJECT}/kernel ] ; then
		...
		cd kernel
		git checkout arch/
		git checkout logo.bmp
		git checkout logo_kernel.bmp
		cd -
	fi
}
