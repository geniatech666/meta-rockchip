...
build_uboot()
{
	...
	if [  -e loong/devices/${BUILD_PROJECT}/u-boot ] ; then
		echo  "\n uboot overlay\n"
		run_command cp -rf -P loong/devices/${BUILD_PROJECT}/u-boot/ ./
	fi
	if [  -e loong/devices/${BUILD_PROJECT}/rkbin ] ; then
		echo  "\n rkbin overlay\n"
		run_command cp -rf -P loong/devices/${BUILD_PROJECT}/rkbin/ ./
	fi
	...
	if [  -e ../loong/devices/${BUILD_PROJECT}/u-boot ] ; then
		echo  "\n uboot checkout\n"
		run_command git checkout arch/arm/dts/
		run_command git checkout configs/
	fi

	if [  -e ../loong/devices/${BUILD_PROJECT}/rkbin ] ; then
		echo  "\n rkbin checkout\n"
		run_command cd ../rkbin/
