...
build_yocto()
{
	...
	if [ ! -d device/rockchip/common/data/yocto/ ]
	then
		mkdir -p device/rockchip/common/data/yocto/
	fi

	if [ -e $RK_OUTDIR/../loong/devices/${BUILD_PROJECT}/yocto-overlay ] ; then
                echo "\n ${BUILD_PROJECT} yocto overlay !\n"
                cp -rf loong/devices/${BUILD_PROJECT}/yocto-overlay/* device/rockchip/common/data/yocto/
        fi
	...
	if [ -e $RK_OUTDIR/../loong/devices/${BUILD_PROJECT}/yocto-overlay ] ; then
                echo "\n ${BUILD_PROJECT} yocto overlay delete!\n"
		rm -rf $RK_OUTDIR/../device/rockchip/common/data/yocto
	fi
...
build_debian()
{
	...
	if [ -e loong/devices/${BUILD_PROJECT}/debian ] ; then
		cp -rf -P loong/devices/${BUILD_PROJECT}/debian ./
		echo "\n ${BUILD_PROJECT}  debian overlay !\n"
	fi
